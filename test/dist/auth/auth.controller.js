...
let AuthController = class AuthController {
    constructor(authService, myLoggerService) {
    ...
    async login(body, res) {
        try {
            ...
            if (!mobile || !password) {
                throw new common_1.BadRequestException('Mobile and password are required');
            }
            ...
            return res.status(common_1.HttpStatus.OK).json({ message: 'Login successful', accessToken: result.accessToken });
        }
        catch (error) {
            await this.myLoggerService.error('Login failed', error.stack);
            const message = error.message || 'Login failed';
            return res.status(common_1.HttpStatus.UNAUTHORIZED).json({ message });
        }
    }
};
